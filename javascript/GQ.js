...
var gq = {
				...
				$.each(words, function(i, verse) {
					if (verse){
						...
						tooltip = '<span class=hotlink grmr><span class=arr>$TOKEN1</span>&nbsp;&nbsp;<span class=w2w>$TOKEN2ESC</span>&nbsp;'+
								  ...
								  '<span class=grammar style=font-size:0.6em; data='+ token3 + ' >' + ( token3_1 ) + /*wordImageLink +*/ '</span></span>';

						verse_template_mouseover = '<span class="word"><span class="ar quranText tipsWord POS-$POS" style="font-size:1em" title="$TOOLTIP">$TOKEN1</span></span>';
	...
	player: {
		...
		preload: false, // true (two players playing continuesly), false (play with one and load with one) or -1 (just play only, no preload)
